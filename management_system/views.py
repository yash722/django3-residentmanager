...
def outgoing_list(request):
    people = Resident.objects.filter(
        cameback__isnull=True, corona_screening_positive=False, user=request.user)
    return render(request, 'management_system/outgoing_entrylist.html', {'people': people})


def outgoing_info(request, person_pk):
    if request.method == "GET":
        person = get_object_or_404(Resident, pk=person_pk)
...
def returned(request):
    people = Resident.objects.filter(
        cameback__isnull=False, corona_screening_positive=False, user=request.user)
...
def isolate(request):
    people = Resident.objects.filter(
        corona_screening_positive=True, user=request.user)
...
def visitor_list(request):
    people = visitor.objects.filter(
        vis_returned_time__isnull=True, user=request.user)
